...
class PlayerModel:
    ...
    def render(self) -> str:
        ...
        bits = [f"{self.name}（好感 {self.affinity}，已发言 {self.utterance_count} 次{where}）"]
        if self.revealed:
...
class StateTracker:
    ...
    def update_from_env(self, observation: dict[str, Any]) -> None:
        ...
        for spec in observation.get("present_actors") or observation.get("visible_actors") or []:
            if spec.get("kind") != "player":
                continue
            seen.add(spec["id"])
            model = self.players.get(spec["id"]) or PlayerModel(spec["id"], spec["name"])
    ...
    def speaking_load(self) -> dict[str, float]:
        ...
        total = self.npc_utterances + sum(p.utterance_count for p in self.players.values())
        if total == 0:
