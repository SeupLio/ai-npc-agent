...
#: 跑批那边早就有重试了，判分一直漏着。
#:
#: ⚠️ 这里从前写着"只对**调用失败**重试，不对解析失败重试：解析失败通常是
#: 确定性的"。**那句是错的，而且已经在下面被推翻了**（见 `_judge_one` 里
#: 那段注释）：思维链长度在不同调用之间波动极大，被预算截断是**随机事件**，
#: 不是这句话的属性。实测 1/792 条命中，重试的成本上限只有 0.25%×2 次额外调用。
#:
#: ⚠️ 2026-09-20 起，解析层多了一层**客户端**重试（`parse_attempts`，附十六）。
#: 于是这里的 `parse_retries` 计数会**偏低** —— 大部分解析失败在客户端层
#: 就被吸收了，根本传不到这个循环里。**这是已知的口径问题**：
#: 要数"整条链路上重试了多少次"，得把客户端的 `parse_retries` 一起看。
JUDGE_MAX_RETRIES = 2
JUDGE_BACKOFF = 3.0
